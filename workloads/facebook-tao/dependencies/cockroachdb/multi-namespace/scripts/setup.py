...
def parse_args():
    ...
    parser.add_argument('--statefulsets-only', action='store_true', help='Only deploy statefulset')
...
def validate_input_and_environment():
...
def create_long_lived_resources():
...
def create_statefulsets(join_str):
...
def main():
    args = parse_args()
    validate_input_and_environment()
    if not args.statefulsets_only:
        create_certs()
        create_secrets()
        # setup_dns_forwarding()    # Not needed in Nautilus' single-cluster setup
        create_service()
        create_long_lived_resources()
    create_statefulsets()
    init_cluster()
    wait_till_ready()
